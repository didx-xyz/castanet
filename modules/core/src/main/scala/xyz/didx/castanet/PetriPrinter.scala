...
case class PetriPrinter(
  ...
  def print(markers: Option[Markers] = None, steps: Option[Map[ArcId, Long]] = None) =
    ...
    val builder: StringBuilder =
      petriNet.elements.foldLeft(new StringBuilder("digraph G {\n"))((b, kv) =>
        b.append(kv._2 match
          case p: Place =>
    ...
    petriNet.graph.foldLeft(builder.append("\n"))((b, kv) =>
      b.append(
        kv._2.toIndexedSeq.reverse
          .map(l =>
            val stp =
              if steps.getOrElse(Map[ArcId, BitVector]()).contains(ArcId(kv._1, l.id)) then
